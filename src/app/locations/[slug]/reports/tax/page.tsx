...
export default async function TaxReportPage({ params, searchParams }: Props) {
  ...
  // Tax filing runs month to month, so this one defaults to the current month rather than 30 days.
  const range = resolveRange(sp, tz, "month");
  const r = await taxReport(loc.id, range.from, range.to);

  return (
    <ReportShell slug={slug} report={reportByKey("tax")!} range={range}>

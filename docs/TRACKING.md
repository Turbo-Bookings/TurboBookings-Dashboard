...
| TRK-26 | **Houston's live site renders Miami's brand colours.** Houston's primary is `#d42427` but 6 files carry Miami's `#c8102e` against 3 with Houston's own; `#C8102E`, `#8A0A1E` and `#39FF14` all appear in production HTML. The fork's colour substitution was partial and went unnoticed because both are reds. `#d4a853` is a false positive — it is genuinely Houston's accent | **open — needs operator sign-off**, it is a visible change to a client's live site | | |
